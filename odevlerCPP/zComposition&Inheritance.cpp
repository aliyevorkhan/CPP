...
public:
      Event(int hours = 0, int minutes = 0, 
            int d = 1, int m = 1, int y = 1900, string name = "Arasinav");
...
{//instantiate an object and set data for Christmas
      Event object;
      //object.setEventData(6, 0, 25, 12, 2010, "Christmas");
...
void Event::setEventData(int hours, int minutes, int d, int m, int y, string name)
{
      eventTime.setTime(hours, minutes);
      eventDay.setDate(d, m, 2000);
      eventName = name;
}
...
class Shape {
   public:
      Shape(int en = 0, int boy = 0){
          width = en;
          height = boy;
          cout << "shape calisti"<<endl;
...
class Rectangle: public Shape {
   public:
       Rectangle( int en, int boy):Shape(en, boy){}
      int getArea() {
         return (width * height);
      }
};

class Ucgen: public Shape{
    public:
      Ucgen(int en, int boy):Shape(en,boy){
          cout << "ucgen calisti"<<endl;
      }
...
int main(void) {
   ...
   cout << "Ucgenin alani: " << uc.getArea()<<endl;
   return 0;
}
*/
//---------------------------
